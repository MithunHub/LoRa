...
node = sx126x.sx126x(serial_num = "/dev/ttyS0",freq=433,addr=80,power=22,rssi=True)
...
def send_cpu_continue(send_to_who,continue_or_not = True):
    ...
    if continue_or_not:
        ...
        day  = strftime("%d %m %y at %I:%M%p")
        node.send(str(day)+" CPU Temperature:"+str(get_cpu_temp())+" C")
        time.sleep(0.2)
        node.set(node.freq,node.addr_temp,node.power,node.rssi)
